...
def generate_transcript_from_pdf(
    ...
    for i, chunk in enumerate(chunks):
        ...
        chunk_transcript = generate_transcript(combined_chunk, client, model, language, format_type=format_type, is_first=is_first, is_last=is_last, style=style, length=length, num_speakers=num_speakers, custom_preferences=custom_preferences)
...
def generate_transcript(
    ...
    length_guides = {
        "short": "Keep the transcript concise and to the point, avoiding unnecessary details.",
        "medium": "Provide a balanced length with enough detail to be informative and engaging.",
...
- If {format_type} is narration, summary, article, or lecture: use 1 speaker.
- If {format_type} is podcast, interview, q-and-a, or tutorial: use 2 speakers.
- If {format_type} is panel-discussion, debate, meeting, or analysis: use 3–6 speakers.
"""
    if num_speakers is not None:
        system_prompt += f"- If num_speakers is provided, always use exactly that many speakers (labeled \"Speaker 1\", \"Speaker 2\", ...).\n"
...
def generate_audio(
    ...
    transcript = generate_transcript_from_pdf(pdf_path, ollama_client, llm_model, language, format_type=format_type, style=style, length=length, num_speakers=num_speakers, custom_preferences=custom_preferences)
